...
int insert_into_sorted(int source[], int source_len, int destination[], int new_elem)
{
    ...
    for (j = 0; j < source_len + 1; j++)
    {
        if (placed == false)
        {
            if (j == source_len)
            {
                destination[j] = new_elem;
                return 1;
            }
            if(new_elem > source[i])
            {
//                cout << new_elem << " bigger than " << source[i] << endl << endl;
                destination[j] = source[i];
                i++;
            }
            else
            {
//                cout << new_elem << " NOT bigger than " << source[i] << endl << endl;
                destination[j] = new_elem;
        ...
        {
//           cout << "has been placed" << endl << endl;
            destination[j] = source[i];
            i++;
        }
    }
